...
/** POST /api/upload — uploads an image or audio file to Supabase Storage, returns { url } */
export async function POST(request: NextRequest) {
  ...
  // MediaRecorder may send "audio/webm;codecs=opus" — strip codec
  const baseType = file.type.split(";")[0]?.trim() ?? file.type;
  if (!ALLOWED_TYPES.includes(baseType))                              return NextResponse.json({ error: "Filtype ikke tillatt" }, { status: 400 });
