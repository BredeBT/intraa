      ...
      <div className="mb-6 rounded-xl border border-violet-500/30 bg-violet-500/5 p-4">
        ...
        <div className="mt-3 grid grid-cols-3 gap-3 text-center text-xs">
          <div className="rounded-lg bg-zinc-900 py-2">
            <p className="font-bold text-white">{me.posts}</p>
            <p className="text-zinc-500">Innlegg</p>
          </div>
          <div className="rounded-lg bg-zinc-900 py-2">
            <p className="font-bold text-white">{me.comments}</p>
            <p className="text-zinc-500">Kommentarer</p>
          </div>
          <div className="rounded-lg bg-zinc-900 py-2">
            <p className="font-bold text-white">{me.likesReceived}</p>
            <p className="text-zinc-500">Likes mottatt</p>
          </div>
        </div>
      </div>
              ...
              return (
                <tr
                  ...
                  className={`transition-colors ${
                    isMe ? "bg-violet-500/5 hover:bg-violet-500/10" : "hover:bg-zinc-900"
                  } ${i < LEADERBOARD.length - 1 ? "border-b border-zinc-800" : ""}`}
                >
                  <td className="px-4 py-3.5">
                    <span className="text-base">{MEDAL[member.rank] ?? `${member.rank}`}</span>
                  </td>
                  <td className="px-4 py-3.5">
                  ...
                  <td className="px-4 py-3.5 hidden sm:table-cell">
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${member.badgeColor}`}>
                      {member.badge}
                    </span>
                  </td>
