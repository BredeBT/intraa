...
const INITIAL_CONTESTS: Contest[] = [
  {
    id: "c1",
    title: "Beste open-source bidrag",
    description: "Del ditt beste open-source prosjekt eller bidrag fra siste kvartal. Vinneren kåres av communityet via stemmer.",
    prize: "5 000 kr gavekort",
  ...
  {
    id: "c2",
    title: "Ukens UI-utfordring",
    description: "Bygg en tilgjengelig dato-velger komponent med kun HTML, CSS og vanilla JS. Ingen biblioteker.",
    prize: "Feature i nyhetsbrevet",
    prizeColor: "text-violet-400",
  ...
  {
    id: "c3",
    title: "Beste innlegg i april",
    description: "Det innlegget med flest genuine likes og engasjement i april vinner. Skriv om noe du er lidenskapelig opptatt av.",
    prize: "VIP-status i 3 måneder",
  ...
  {
    id: "c4",
    title: "Mai-hackathon: AI-verktøy",
    description: "Bygg et nyttig AI-drevet verktøy på under 48 timer. Bli med et team eller delta alene. Mer info kommer.",
    prize: "10 000 kr + mentoring",
  ...
  {
    id: "c5",
    title: "Beste tutorial — mars",
    description: "Avsluttet konkurranse. Vinneren var Kari Moe med sin tutorial om tilgjengelige fargepaletter.",
    prize: "5 000 kr gavekort",
    prizeColor: "text-zinc-500",
...
export default function KonkurranserPage() {
  ...
  function toggleJoin(id: string) {
    ...
    setContests(prev => prev.map(c =>
      c.id === id ? { ...c, participants: c.participants + (joining ? 1 : -1) } : c
    ));
  }
      ...
      <div className={`flex flex-col rounded-2xl border bg-zinc-900 p-5 transition-colors ${isEnded ? "border-zinc-800 opacity-60" : "border-zinc-800 hover:border-zinc-700"}`}>
        <div className="mb-3 flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_CONFIG[contest.status].style}`}>
        ...
        <div className="mb-4 flex items-center gap-4 text-sm">
          <div className="flex items-center gap-1.5">
            ...
            <span className={`font-medium ${contest.prizeColor}`}>{contest.prize}</span>
          </div>
          <div className="flex items-center gap-1.5 text-zinc-500">
            <Users className="h-3.5 w-3.5" />
            {contest.participants} deltakere
          </div>
        </div>

        {!isEnded && (
          <button
            onClick={() => toggleJoin(contest.id)}
          ...
          >
            {joined[contest.id] ? (
              <><CheckCircle className="h-4 w-4" /> Påmeldt — klikk for å melde av</>
            ) : (
              "Delta"
            )}
          </button>
        ...
        {isEnded && (
          <div className="rounded-xl bg-zinc-800/50 py-2.5 text-center text-sm text-zinc-500">
    ...
    <div className="px-8 py-8">
      ...
      {active.length > 0 && (
        <section className="mb-8">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-zinc-500">Aktive</h2>
      ...
      {upcoming.length > 0 && (
        <section className="mb-8">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-zinc-500">Kommende</h2>
          <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
      ...
      {ended.length > 0 && (
        <section>
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-zinc-500">Avsluttede</h2>
          <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
